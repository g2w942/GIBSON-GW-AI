...
# Live internet access module (100% Free)
try:
...
<div class="avatar-matrix">
    <div style="display: flex; align-items: center; gap: 18px;">
        ...
        <div>
            ...
            <p style="margin: 3px 0 0 0; font-size: 11px; color: #a0aec0; letter-spacing: 0.5px;">STATUS: ACTIVE // 1M TOKEN ENGINE // LIVE WEB ON</p>
        </div>
    </div>
</div>
""", unsafe_allow_html=True)

# API Configuration via Sidebar Terminal
st.sidebar.header("🛸 SYSTEM CONSOLE")
st.sidebar.write("Authenticate parsing tracks using your Free Google Gemini API Key (1M Token Limit).")
gemini_api_key = st.sidebar.text_input("Gemini API Key:", type="password")
st.sidebar.markdown("[Get Free Key Here](https://aistudio.google.com/app/apikey)")

# LIVE INTERNET FETCH MODULE (Zero limits, blocks Wikipedia)
    ...
    try:
        ...
        if file_ext == "txt":
            ...
            if pypdf is not None:
                ...
                st.error("Engine failure: 'pypdf' dependency is not mounted.")
...
if prompt := st.chat_input("Input command to GIBSON GW..."):
    ...
    if gemini_api_key:
        try:
            # Initialize Gemini Engine (Zero Token Errors)
            genai.configure(api_key=gemini_api_key)
            model = genai.GenerativeModel('gemini-1.5-flash')

            with st.chat_message("assistant", avatar="👩‍💻"):
                st.write("**EDITH:** Activating live internet crawler & bypassing Wikipedia domains...")
                ...
                execution_prompt = (
                    f"You are GIBSON GW, an elite AI collective managed by EDITH. "
                    f"You have a 1,000,000 token context window. Synthesize the data flawlessly.\n\n"
                )
                
                execution_prompt += f"--- LIVE INTERNET DATA ---\n{live_web_data}\n--- END INTERNET DATA ---\n\n"
                ...
                execution_prompt += (
                    ...
                    f"STRICT SYSTEM DIRECTIVE: You MUST use the provided live internet data and file data to construct your response. "
                    f"CRITICAL: You MUST include highly accurate APA 7th Edition inline citations throughout your text. "
                    f"CRITICAL: You MUST provide a fully formatted 'References' list at the very bottom of the response. "
                    f"CRITICAL: NEVER use Wikipedia as a source. Rely strictly on academic, news, or definitive web entries provided. "
                    f"Use varied sentence configurations, natural transitional phrasing, and adaptive human flow mechanics to entirely bypass mechanical tracking engines. "
                    f"Append a section titled exactly '💡 **SUGGESTED NEXT STEPS:**' showing exactly two recommendations wrapped inside [Brackets]."
                )

                with st.spinner("⚡ Fusing consensus structural outputs & compiling APA citations..."):
            ...
            st.error(f"Core Engine Exception: Ensure your Gemini API Key is correct. Error Details: {str(e)}")
    else:
...
if st.session_state.last_compiled_output:
    ...
    if mutation_selected and gemini_api_key:
        try:
            genai.configure(api_key=gemini_api_key)
            mutator_model = genai.GenerativeModel('gemini-1.5-flash')
            
            if mutation_selected == "paraphrase":
                directive_text = "Completely paraphrase the text below while STRICTLY maintaining all APA inline citations and the References section at the bottom. Use extensive vocabulary replacements."
            elif mutation_selected == "humanize":
                directive_text = "Completely humanize the text below. Alter sentence lengths systematically, embed conversational flow, but DO NOT modify or remove the APA citations or the References section."
            else:
                directive_text = "Execute a complete structural overhaul, paraphrase rewrite, and humanization process. You MUST KEEP all APA inline citations and the References section entirely intact."

            complete_mutation_prompt = f"{directive_text}\n\nTARGET CORE TEXT:\n{st.session_state.last_compiled_output}\n\nMutated Core Output:"
            
            with st.spinner("⚡ Transmuting core structural string matrices..."):
                mutation_response = mutator_model.generate_content(complete_mutation_prompt)
                mutated_result = mutation_response.text
